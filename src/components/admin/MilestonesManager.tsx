...
export function MilestonesManager({ token }: { token: string }) {
  ...
  const save = async () => {
    try {
      ...
      await load();
      setMessage(
        "Hito guardado. La línea de tiempo se actualizará según su fecha.",
      );
    } catch (error) {
  ...
  const remove = async () => {
    if (!selectedId || !window.confirm("¿Eliminar este hito y su publicación?"))
      return;
    await apiRequest(
    ...
    <div className={styles.manager}>
      <aside className={styles.listPanel}>
        <div className={styles.listHeader}>
          <h2>Hitos</h2>
          <p>Archivo visual organizado automáticamente por meses.</p>
          <button
        ...
        <div className={styles.itemList}>
          ...
          {!filtered.length && (
            <div className={styles.empty}>
              Crea el primer momento de la historia del capítulo.
            </div>
          )}
        </div>
      </aside>
      ...
      <section className={styles.editor}>
        <div className={styles.editorHeader}>
          <div>
            <h2>{selectedId ? "Editar hito" : "Nuevo hito"}</h2>
            <p>La fecha determina el mes y el orden en la página pública.</p>
          </div>
        ...
        <div className={styles.formGrid}>
          ...
          <ImageField
            ...
            help="Recomendado: formato horizontal y alta resolución. Se conserva mucha más calidad y nunca se recorta en la vista pública."
            required
          />
          ...
          <div className={styles.threeColumns}>
            ...
            <Field
              ...
              placeholder="Evento, alianza, reconocimiento…"
            />
            <Field
              label="Lugar"
              value={form.location ?? ""}
